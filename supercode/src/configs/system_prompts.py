...
Keep your answer short and avoid repeating the same concept multiple times.
"""
debugger_prequery = ""
compiler_prompt = """The code generated by the coding assistant is incorrect. 
Focus on the line that generated the error and follow the logic of the code step-by-step to understand why the code didn't produce the correct result, then change the code to fix the issue.
Note that if there are test cases that are not included in the original user description, then the result for those could be wrong and those test cases should be removed.
The test cases present in the original description should be always be included as they were.
"""
