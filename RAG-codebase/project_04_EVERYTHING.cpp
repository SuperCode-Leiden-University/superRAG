...
void energy_coord(double, double, double, double, double &, double &, double &,
                  double &, double &);
...
static double g_L1 = 1., g_m1 = 1., g_L2 = 1.,
              g_m2 = 1.; // length and mass of the pendulum
...
int main() {
  ...
  double x1, x2, y1, y2, v1, v2;      // position in space (cartesian coord) and
                                      // angular speed times length of the rod
...
// -----------------------------------------------------------------------------------
  ...
  double Y0[neq] = {M_PI * 0.25, M_PI * 0.25, 0.,
                    0.}; // Y0={theta1, theta2, v_theta1, v_theta2} -> initial
                         // values (border condition), R=dY/dt
  // NB: thetas are calculated from the vertical, if theta1=theta2=2*pi/4, then
  // the centre and the masses form a line parallel to the ground at time t=0
  ...
  for (int j = 1; j <= 2; j++) {
    for (int i = 0; i < neq; i++)
      Y[i] = Y0[i]; // Y must contein initial values when you start integrating
    t = 0;          // starting time
    for (int i = 0; i < N; i++) {
      ...
      for (int l = 0; l < n_var; l++)
        NormAng(Y[l]); // this seems useless, but if I plot the graph I want
                       // thetas to stay in [ -pi, +pi ]
  ...
  for (int j = 1; j <= 2; j++) {
    for (int i = 0; i < n_var; i++) {
      X[i] =
          Y0[i]; // X,V must contein initial values when you start integrating
      V[i] = Y0[i + n_var];
    }
    t = 0; // starting time
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int l = 0; l < n_var; l++)
        NormAng(X[l]); // this seems useless, but if I plot the graph I want
                       // thetas to stay in [ -pi, +pi ]
...
// -----------------------------------------------------------------------------------
  ...
  for (int j = 1; j <= 2; j++) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (i == 0)
        E0 = E;
      energy_coord(Ysol[0], Ysol[1], Ysol[2], Ysol[3], x1_sol, x2_sol, y1_sol,
                   y2_sol, E_sol);

      fdata << t << " " << dt << " " << E - E_sol << " " << Y[0] - Ysol[0]
            << " " << Y[1] - Ysol[1] << " " << Y[2] - Ysol[2] << " "
            << Y[3] - Ysol[3] << " " << E << " " << E_sol << " " << Y[0] << " "
            << Ysol[0] << " " << Y[1] << " " << Ysol[1] << " " << Y[2] << " "
            << Ysol[2] << " " << Y[3] << " " << Ysol[3] << endl;
      // the last line is useful to check the sovrapposion in a
      // qualitative/intuitive way
    }
  ...
  for (int j = 1; j <= 3; j++) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      fdata << t << " " << dt << " " << E - E_sol << " " << X[0] - Ysol[0]
            << " " << X[1] - Ysol[1] << " " << V[0] - Ysol[2] << " "
            << V[1] - Ysol[3] << " " << E << " " << E_sol << " " << X[0] << " "
            << Ysol[0] << " " << X[1] << " " << Ysol[1] << " " << V[0] << " "
            << Ysol[2] << " " << V[1] << " " << Ysol[3] << endl;
      // the last line is useful to check the sovrapposion in a
      // qualitative/intuitive way
    }
    fdata << "\n" << endl;
...
// -----------------------------------------------------------------------------------
  ...
  for (int j = 1; j <= 2; j++) {
    ...
    for (int N = N_min; N < N_max; N = N * N_step) {
      ...
      dt = t_max / (double)N;
      if (j == 1)
        cout << " --- dt=" << dt << endl; // printing dt only the first time

      for (int i = 0; i < neq; i++)
        Y[i] =
            Y0[i]; // Y must contein initial values when you start integrating
      t = 0;       // starting time
      ...
      fdata << t << " " << dt << " " << fabs(E - E_sol) << " "
            << fabs(Y[0] - Ysol[0]) << " " << fabs(Y[1] - Ysol[1]) << " "
            << fabs(Y[2] - Ysol[2]) << " " << fabs(Y[3] - Ysol[3]) << endl;
    }
    fdata << "\n" << endl;
  }
  ...
  for (int j = 1; j <= 3; j++) {
    ...
    for (int N = N_min; N < N_max; N = N * N_step) {
      ...
      fdata << t << " " << dt << " " << fabs(E - E_sol) << " "
            << fabs(X[0] - Ysol[0]) << " " << fabs(X[1] - Ysol[1]) << " "
            << fabs(V[0] - Ysol[2]) << " " << fabs(V[1] - Ysol[3]) << endl;
    }
    fdata << "\n" << endl;
  }
...
// -----------------------------------------------------------------------------------
  ...
  for (int a = 0; a < N_ang; a++) {
    Ya0[0] = (2 * (double)a / (double)N_ang - 1) *
             M_PI; // starts from -pi and ends in +pi
    Yb0[0] = Ya0[0] + fluc;
    ...
    for (int b = 0; b < N_ang; b++) {
      ...
      // RK4 method ---------------------
      for (int i = 0; i < neq;
           i++) { // Y must contein initial values when you start integrating
        Ya[i] = Ya0[i];
        Yb[i] = Yb0[i];
      }
      t = 0; // starting time
      ...
      for (int i = 0; i < N; i++) {
        ...
        if (fabs(Ya[0] - M_PI) < 1. or
            fabs(Ya[1] - M_PI) <
                1.) { // only checking when the angle is close to pi
          if ((Ya[0] - M_PI) * (theta_prec[0] - M_PI) < 0. or
              (Ya[1] - M_PI) * (theta_prec[1] - M_PI) <
                  0.) { // one of mass flip
            // if ( (Ya[1]-M_PI)*(theta_prec[1]-M_PI) < 0. ) { // checking only
            // when m2 flips, alternative
            flip = true;
            k++;
          }
        }
        ...
        for (int l = 0; l < n_var; l++)
          NormAng(Ya[l]); // this seems useless, but if I plot the graph I want
                          // thetas to stay in [ -pi, +pi ]
        for (int l = 0; l < n_var; l++)
          NormAng(Yb[l]);
        ...
        if (not ok_flip and i == N - 1)
          fdata_flip << t << " " << Ya0[0] << " " << Ya0[1]
                     << endl; // flip didn't occur
        ...
        if (not ok_rand and i == N - 1)
          fdata_rand << t << " " << Ya0[0] << " " << Ya0[1] << endl;

        // if both flip and divergence happened already, then skip to the next
...
// -----------------------------------------------------------------------------------
  ...
  for (int a = 0; a < N_ang; a++) {
    ...
    for (int b = 0; b < N_ang; b++) {
      ...
      // fdata_chaos << delta_mean[0]+delta_mean[1])*0.5 <<" "<< Ya0[0] <<" "<<
      // Ya0[1] << endl;
      fdata_chaos << Ya0[0] << " " << Ya0[1] << " " << delta_mean[0] << " "
                  << delta_mean[1] << " "
                  << (delta_mean[0] + delta_mean[1]) * 0.5 << " " << Ya[0]
                  << " " << Ya[1] << " " << Ya[2] << " " << Ya[3] << " "
                  << Yb[0] << " " << Yb[1] << " " << Yb[2] << " " << Yb[3]
                  << " " << endl;
      // -------------------------------------------------------------------------------
...
void dYdt(double t, double *Y, double *R) {
  ...
  R[3] =
      ( // dd_theta2
          2 * sin(theta1 - theta2) *
          (d_theta1 * d_theta1 * L1 * (m1 + m2) + g * (m1 + m2) * cos(theta1) +
...
void solution(double t, double *Y0, double *Ysol) {
  ...
  Ysol[2] = -(omega1 * A1 * sin(omega1 * t) + omega2 * A2 * sin(omega2 * t)) /
            sqrt(2);
...
// recurrent operation: this will help with readability and lower the chance of
// error
void energy_coord(double theta1, double theta2, double omega1, double omega2,
                  double &x1, double &x2, double &y1, double &y2, double &E) {
  ...
  E = 0.5 * ((m1 + m2) * v1 * v1 + m2 * v2 * v2) +
      m2 * cos(theta1 - theta2) * v1 * v2 + g * (m1 * y1 + m2 * y2);
}
